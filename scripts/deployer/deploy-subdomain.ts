...
export async function deployFakeLoot() {
  ...
  const subdomain = await subdomainENS.deploy(GOERLI_ENS_ADDRESS,GOERLI_ENSR_ADDRESS);

  await subdomain.deployed();

  console.log("test deployed to:", subdomain.address);
  console.log(`npx hardhat verify --network goerli ${subdomain.address} ${GOERLI_ENS_ADDRESS} ${GOERLI_ENSR_ADDRESS}`)
}