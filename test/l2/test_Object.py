...
async def test_balance_of_batch(object_factory):
    ...
    await signer.send_transaction(account, object.contract_address, '_mint_batch', [account.contract_address, 2, 1, 2, 2,1000,500])
...
async def test_transfer_from(object_factory):
    ...
    balance_1_of_other = await object.balance_of(operator.contract_address, 1).call()
    balance_1_of_from_address = await object.balance_of(account.contract_address, 1).call()
    assert balance_1_of_other.result.res == 0

    # Test if Other transfers 1 from Account without approval
    try:
        await other.send_transaction(operator, object.contract_address, 'safe_transfer_from', [account.contract_address, operator.contract_address, 1, 1])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
    ...
    balance_2_of_other = await object.balance_of(operator.contract_address, 1).call()
    assert balance_2_of_other.result.res == balance_1_of_other.result.res + 1
    balance_2_of_from_address = await object.balance_of(account.contract_address, 1).call()
    ...
    try:
        await other.send_transaction(operator, object.contract_address, 'safe_transfer_from', [account.contract_address, operator.contract_address, 1, balance_1_of_from_address.result.res])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
...
async def test_transfer_batch_from(object_factory):
    ...
    balances_1 = (await object.balance_of_batch([account.contract_address, account.contract_address, operator.contract_address, operator.contract_address], [1, 2, 1, 2]).call()).result.res
    ...
    try:
        await other.send_transaction(operator, object.contract_address, 'safe_batch_transfer_from', [account.contract_address, operator.contract_address, 2, 1, 2, 2, amount1, amount2])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
    ...
    balances_2 = (await object.balance_of_batch([account.contract_address, account.contract_address, operator.contract_address, operator.contract_address], [1, 2, 1, 2]).call()).result.res
    # Balance 2 = balance 1 with operations made
    assert balances_2 == [x+y for x, y in zip(balances_1, operations)]

    # OTHER TAKE TOO MUCH
    try:
        await other.send_transaction(operator, object.contract_address, 'safe_batch_transfer_from', [account.contract_address, operator.contract_address, 2, 1, 2, 2, 1000, 1000])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
...
async def test_burn(object_factory):
    ...
    # burn 10 tokens
    balance_before = (await object.balance_of(account.contract_address, token_id).call()).result.res
    await signer.send_transaction(account, object.contract_address, '_burn', [account.contract_address, token_id, amount_to_burn])
    assert (await object.balance_of(account.contract_address, token_id).call()).result.res == balance_before - amount_to_burn

    # try burning too much tokens
    try:
        await signer.send_transaction(account, object.contract_address, '_burn', [account.contract_address, token_id, 5000])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
...
async def test_burn_batch(object_factory):
    ...
    balances_1 = (await object.balance_of_batch([account.contract_address, account.contract_address], [1, 2]).call()).result.res
    # amounts to transfer between balances
    amount1 = 50
    amount2 = 25
    ...
    await signer.send_transaction(account, object.contract_address, '_burn_batch', [account.contract_address, 2, 1, 2, 2, amount1, amount2])

    balances_2 = (await object.balance_of_batch([account.contract_address, account.contract_address], [1, 2]).call()).result.res
    # Balance 2 = balance 1 with operations made
    assert balances_2 == [x+y for x, y in zip(balances_1, operations)]

    # Other burns more tokens than current balance
    try:
        await other.send_transaction(account, object.contract_address, '_burn_batch', [account.contract_address, 2, 1, 2, 2, 1000, 1000])
    except StarkException as err:
        _, error = err.args
        assert error['code'] == StarknetErrorCode.TRANSACTION_FAILED
